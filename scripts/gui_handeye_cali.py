...
LastEditTime: 2023-01-24 11:06:59
Description: 
...
class CalibGUI(QMainWindow):
    ...
    def show_aruco_res(self, use_charuco=True):  # TODO
        self.aruco_res = self.aruco_detector.get_aruco_detect_res(use_charuco=use_charuco)
        ...
        if self.aruco_res.keys():
            if use_charuco:
                astr = '<b>Charuco pose (px, py, pz, qx, qy, qz, qw):</b>'
                self.sampler.ar_trans, self.sampler.ar_quat  = self.aruco_res['charuco']
